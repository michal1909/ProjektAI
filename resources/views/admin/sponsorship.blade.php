                ...
                @foreach($sponsorships as $sponsorship)
                    <tr>
                        <form action="{{route('admin.updateSponsorship')}}" id="myform{{$sponsorship->id}}" method="post">
                            @csrf
                            <input class="form-control" type="text" name="id" hidden value="{{$sponsorship->id}}">
                            <input class="form-control" type="text" name="sponsor_id" hidden value="{{$sponsorship->sponsor_id}}">
                            <input class="form-control" type="text" name="animal_id" hidden value="{{$sponsorship->animal_id}}">
                            <td>{{$sponsorship->sponsor->name}}</td>
                            <td>{{$sponsorship->animal->name}}</td>
                            <td><input class="form-control" type="text" name="contribution" value="{{$sponsorship->contribution}}"></td>
                        </form>
                        <td>
        ...
        <div class="w-50">
            <form method="POST"  action="{{route('admin.createSponsorship')}}"  enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <input class="form-control" type="number" name="sponsor_id" min="1"><br>
                </div>
                <div>
                    <label class="mb-2 mt-3 fw-bold" for="animal_id">{{__('Id zwierzęcia:')}}</label><br>
                    <input class="form-control" type="number" name="animal_id" min="1"><br>
                </div>
                <div>
                    <label class="mb-2 mt-3 fw-bold" for="contribution">{{__('Wkład:')}}</label><br>
                    <input class="form-control" type="number" name="contribution"><br>
                </div>
